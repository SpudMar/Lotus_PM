 ...
 *   - 404 invoice has no s3Key (NO_DOCUMENT)
 *   - 200 success with presigned URL
 *   - presigned URL generated with correct 15-minute expiry
 */
...
const mockSession = {
  user: { id: 'user-001', role: 'PLAN_MANAGER', email: 'pm@lotus.com', name: 'James Walker' },
}
...
function makeInvoice(overrides: Record<string, unknown> = {}) {
  return {
    ...
    s3Key: 'uploads/manual/1706000000000-abc123.pdf',
    s3Bucket: 'lotus-pm-invoices-staging',
...
function makeRequest(id: string): NextRequest {
  return new NextRequest(`http://localhost/api/invoices/${id}/pdf`)
}
...
describe('GET /api/invoices/[id]/pdf', () => {
  ...
  it('returns 404 when invoice not found', async () => {
    ...
    const res = await GET(makeRequest('inv-notexist'), {
  ...
  it('returns 404 when invoice has s3Key but no s3Bucket', async () => {
    mockRequirePermission.mockResolvedValue(mockSession as never)
    mockGetInvoice.mockResolvedValue(
      makeInvoice({ s3Key: 'uploads/manual/abc.pdf', s3Bucket: null }) as never,
    )
  ...
  it('returns 200 with presigned URL on success', async () => {
    ...
    mockGetSignedUrl.mockResolvedValue(
      'https://s3.ap-southeast-2.amazonaws.com/bucket/key?signed=true',
    )

    const res = await GET(makeRequest('inv-001'), { params: Promise.resolve({ id: 'inv-001' }) })

    expect(res.status).toBe(200)
    const body = await res.json() as { data: { url: string; expiresIn: number } }
    expect(body.data.url).toBe('https://s3.ap-southeast-2.amazonaws.com/bucket/key?signed=true')
    expect(body.data.expiresIn).toBe(900) // 15 minutes
  })

  it('generates presigned URL with 15 minute (900s) expiry', async () => {
    mockRequirePermission.mockResolvedValue(mockSession as never)
    mockGetInvoice.mockResolvedValue(makeInvoice() as never)
  ...
  it('uses correct bucket and key from invoice record', async () => {
    const invoice = makeInvoice({
      s3Key: 'uploads/manual/1706000000000-xyz.pdf',
      s3Bucket: 'lotus-pm-invoices-production',
    })
    mockRequirePermission.mockResolvedValue(mockSession as never)
    mockGetInvoice.mockResolvedValue(invoice as never)
    mockGetSignedUrl.mockResolvedValue('https://example.com/signed')

    await GET(makeRequest('inv-001'), { params: Promise.resolve({ id: 'inv-001' }) })

    // GetObjectCommand should have been called with the correct bucket and key
    const { GetObjectCommand } = jest.requireMock('@aws-sdk/client-s3') as {
      GetObjectCommand: jest.Mock
    }
    expect(GetObjectCommand).toHaveBeenCalledWith({
      Bucket: 'lotus-pm-invoices-production',
      Key: 'uploads/manual/1706000000000-xyz.pdf',
    })
  })
})
